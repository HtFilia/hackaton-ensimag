...
PRIVATE_FIXTURES = sorted(Path("tests/fixtures_private/level5").glob("*.yaml"))
...
def test_team_level5_public(team, fixture):
    passed, message = validate_level(team, "level5", fixture)
    assert passed, message


@pytest.mark.parametrize("team", get_teams())
@pytest.mark.parametrize("fixture", PRIVATE_FIXTURES, ids=lambda p: p.stem)
def test_team_level5_private(team, fixture):
    passed, message = validate_level(team, "level5", fixture)
    assert passed, message
